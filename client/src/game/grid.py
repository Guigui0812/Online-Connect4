...
class Grid:
    ...
    def get_serialized_matrix(self):
        ...
        DictOfserializedObject = {"box_status_matrix": pickle.dumps(
            self.box_status_matrix), "max_column_stacking": pickle.dumps(self.max_column_stacking)}
    ...
    def draw_triangle(self, screen, x):
        ...
        for j in range(COLUMN_COUNT):
            ...
            if self.visual_matrix[self.max_column_stacking[j]][j].rect.x < x < self.visual_matrix[self.max_column_stacking[j]][j].rect.x + self.visual_matrix[self.max_column_stacking[j]][j].rect.width:
                triangle_points = [(self.visual_matrix[self.max_column_stacking[j]][j].rect.x + (self.visual_matrix[self.max_column_stacking[j]][j].rect.width/2), 100), 
                                (self.visual_matrix[self.max_column_stacking[j]][j].rect.x + 20, 80), ((self.visual_matrix[self.max_column_stacking[j]][j].rect.x 
